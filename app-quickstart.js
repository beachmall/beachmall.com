...
//							$('.numRecentlyViewed','#appView').text(app.ext.quickstart.vars.session.recentlyViewedItems.length);
//							}
...
//appPreViewBG is an optional element used to create a layer between the preView and the view when the preView is loaded 'over' the view.
					...
					_app.ext.quickstart.vars.showContentFinished = true;
//beachmall			if(infoObj.performTransition == false)	{
					_app.ext.quickstart.vars.showContentFinished = true;
					}
...
/*BEACHMART*/			else if(infoObj.pageType == 'category' && $old.data('templateid') == 'categoryTemplateBrands' && $old.data('catsafeid') == infoObj.navcat){r = false; dump("transition suppressed: reloading same brand category");}
...
/*beachmart*/			else { $nav.append("<li><a href='#!company/"+$(this).attr('id').replace('Article','')+"'>"+($('h1:first',$(this)).text())+"</a></li>"); }
						});
...
// 201405 - Depreciated for Universal Analytics
